...
func (d *Database) CreateOriginUser(user *model.User, origin *model.Origin) error {
	tx := d.DB.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
...
func (d *Database) GetOriginUser(email string, password string) (*model.User, error) {
	user := new(model.User)
...
func (d *Database) CreateSocial(social *model.Social) error {
	return d.DB.Create(social).Error
}

func (d *Database) IsEmailUsed(email string) (bool, error) {
	user := new(model.User)
...
func (d *Database) GetUserFromSocialByEmail(email string, socialType int) (*model.User, error) {
	user := new(model.User)
...
func (d *Database) SetUserTokenPair(userTokenPair *model.UserTokenPair) error {
	err := d.DB.Create(userTokenPair).Error
	return err
}

func (d *Database) GetAllUserToken(uuid uuid.UUID) (*[]model.UserTokenPair, error) {
